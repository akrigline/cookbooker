...
Lets a user export a whole cookbook — its settings, chapters in order, and each
chapter's recipes in order — as a single self-contained `.html` file, so a whole
cookbook (not just one recipe) can be shared or moved to another device outside
the app, and later re-imported through `cookbook-import` with full fidelity.
