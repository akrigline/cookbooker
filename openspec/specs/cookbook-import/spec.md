...
Lets a user import a `cookbook/1` HTML file (produced by `cookbook-export`) as
a brand-new cookbook — its own project, chapters, and recipes — without ever
matching or deduping against the existing recipe library, and without
modifying any existing cookbook or recipe. A recipe that fails to parse is
