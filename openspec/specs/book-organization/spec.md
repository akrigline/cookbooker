...
The user SHALL be able to remove a recipe from a specific cookbook, but the system MUST require confirmation before performing the removal. Removing a recipe from a cookbook MUST NOT delete it from other cookbooks or from the central Global Recipe Library.
