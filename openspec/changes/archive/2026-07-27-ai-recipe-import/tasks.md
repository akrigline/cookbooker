...
- [x] 1.1 Create `src/js/recipeImport.js` exporting a pure function (e.g.
      ...
      element.
- [x] 1.2 If zero `[data-cm-format="recipe"]` elements are found anywhere in the
...
- [x] 1.3 For each matched element, gate on `data-cm-version="1"`; treat a
      missing/mismatched version as a per-recipe failure, not a guess-parse.
- [x] 1.4 Extract `title` from `.cm-title` textContent (trimmed).
- [x] 1.5 Extract ingredient lines from `.cm-ingredients li` textContent (trimmed,
      ...
      ingredient parsing.
- [x] 1.6 Extract instruction steps from `.cm-instructions li` textContent, falling
      back to `.cm-instructions p` when no `li` is present; join into a
      newline-separated string.
- [x] 1.7 Extract optional `notes` from `.cm-notes` textContent, and optional
      ...
      `'standard'` otherwise.
- [x] 1.8 Always set `image: null` on every extracted candidate recipe.
- [x] 1.9 Apply the same required-field validation as `RecipeEditor.vue` (non-empty
...
- [x] 2.1 Add `src/js/recipeImport.test.js` following the `describe`/`it` pattern of
      `src/js/conversions.test.js`.
- [x] 2.2 Test: a well-formed single-recipe file parses to one recipe with correct
      title/ingredients/instructions and `image: null`.
- [x] 2.3 Test: a well-formed batch file with multiple `<article class="cm-recipe">`
      blocks parses to multiple recipes.
- [x] 2.4 Test: a file missing the `data-cm-format="recipe"` marker entirely is
      rejected (empty `recipes`, populated top-level/failure reason).
- [x] 2.5 Test: a recipe missing a required field (e.g. empty title, or no
...
- [x] 2.6 Test: instructions extraction falls back to `<p>` elements when no `<li>`
      is present in `.cm-instructions`.
- [x] 2.7 Test: an unrecognized/missing `data-cm-version` is treated as a per-recipe
      failure, not a hard parse of the content.
- [x] 2.8 Run `npm test` and confirm the full suite (existing + new) passes.

## 3. Staged review view

- [x] 3.1 Create a new `src/views/RecipeImport.vue` view: a hidden
      `<input type="file">` (following the `Settings.vue` trigger-button pattern),
      accepting one or more `.html` files.
- [x] 3.2 On file selection, read each file's text (`File.text()`), run it through
      the parser module, and merge results across files with zero DB writes.
- [x] 3.3 Render each successfully parsed recipe using the existing `RecipeSheet.vue`
      component, paired with an include/exclude checkbox defaulting to checked.
- [x] 3.4 Render parse failures in a separate list, each with its reason string.
- [x] 3.5 Add an "Import N recipes" confirm action that calls
...
- [x] 3.6 Handle the whole-file-rejected case (no valid markers found) by showing an
      error message and no importable recipes, without crashing.

## 4. Entry point wiring

- [x] 4.1 Register a new route (e.g. `/library/import`, name `recipe-import`) for
      `RecipeImport.vue` in `src/router/index.js`, alongside the other `/library/*`
      routes.
- [x] 4.2 Add an "Import Recipes" action/link to `RecipeLibrary.vue`'s toolbar, next
      to the existing "+ New Recipe" link.

## 5. LLM prompt

- [x] 5.1 Add the paste-ready LLM prompt text (from the design report §3) somewhere
      the user can copy it from within the import flow (e.g. a "Copy prompt" button
      or a static help block on `RecipeImport.vue`).

## 6. Verification

- [x] 6.1 Run `npm run build` and confirm it succeeds.
- [x] 6.2 Run `npm run dev` and use chrome-devtools-axi to walk through: importing a
...
- [x] 6.3 Walk through importing a batch HTML file with multiple recipes end to end,
      including excluding one recipe via its checkbox and confirming only the
      checked ones are created.
- [x] 6.4 Walk through selecting an invalid file (missing the required markers) and
      confirm the review screen shows a clear error rather than crashing.
- [x] 6.5 Confirm existing recipe library/editor functionality is unaffected
