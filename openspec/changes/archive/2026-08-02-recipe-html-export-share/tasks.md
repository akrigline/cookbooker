...
- [x] 1.1 In `src/js/recipeExport.js` (new file), add `blobToDataUri(blob)`: read
...
- [x] 1.2 In `src/js/recipeImport.js`, add `dataUriToBlob(dataUri)`: parse the mime
...
- [x] 2.1 In `src/js/recipeExport.js`, add `exportRecipeToHtml(recipe)` (async):
...
- [x] 2.2 Serialize `.cm-title` (HTML-escaped), `.cm-ingredients` (one `<li>` per
...
- [x] 2.3 Serialize `<meta class="cm-layout" content="...">` from `layoutTemplate`,
      `<meta class="cm-ingredient-columns" content="...">` from `ingredientColumns`,
      `<meta class="cm-ingredient-qty-align" content="...">` from
      `ingredientQtyAlign`.
- [x] 2.4 When `recipe.image` is present, await `blobToDataUri(recipe.image)` and
...
- [x] 3.1 In `src/js/recipeImport.js`, add extraction of `.cm-image`'s `src` via
      `dataUriToBlob`, defaulting to `null` when the element is absent or decoding
      fails.
- [x] 3.2 Add extraction of `.cm-ingredient-columns`, `.cm-ingredient-qty-align`,
...
- [x] 3.3 Wire these four new fields (`image`, `ingredientColumns`,
      `ingredientQtyAlign`, `imageAspectRatio`) into the object returned by
      `parseRecipeElement`.

## 4. Editor UI

- [x] 4.1 In `src/views/RecipeEditor.vue`, add an "Export recipe" button to the
      existing `isEditing`-gated action row, next to the Print button.
- [x] 4.2 On click: assemble the current recipe fields (same shape used for save),
...
- [x] 4.3 Name the downloaded file `<slugified-title>.html`.

## 5. Tests

- [x] 5.1 New `src/js/recipeExport.test.js`: unit-test `blobToDataUri` round-trips
      a small `Blob`'s bytes and mime type correctly.
- [x] 5.2 In `src/js/recipeExport.test.js`, round-trip test: build a recipe object
...
- [x] 5.3 In `src/js/recipeImport.test.js`, add a backward-compatibility test:
...
- [x] 5.4 In `src/js/recipeImport.test.js`, add a malformed-image test: a
      `.cm-image` with an invalid `data:` URI produces a candidate recipe with
      `image: null` rather than a parse failure.
- [x] 5.5 Run `npm test` and `npm run build`; both must be green.

## 6. Manual Verification

- [x] 6.1 In the running app, export a recipe with a photo, non-default layout,
...
- [x] 6.2 Import that same file via `RecipeImport.vue`'s file picker; confirm the
      staged preview matches the original recipe exactly, including the photo.
- [x] 6.3 Paste the same file's contents into the paste entry point; confirm
      identical results.
- [x] 6.4 Export a recipe with no photo; confirm the downloaded file has no
      `.cm-image` element and re-imports with a null image.
- [x] 6.5 Confirm an existing AI-transcribed HTML file (no new elements) still
