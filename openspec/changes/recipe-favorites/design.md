...
- **`fitsOnPage` measurement can under-report overflow for a favorited recipe in a cookbook with a long title-prefix terminology.** `src/js/recipeFitMeasure.js`'s off-screen mount has no `project` to measure against — it always measures with `getFavoriteSettings(null)` (heart, no prefix), never the terminology prefix a specific cookbook actually renders. This is a genuine instance of the "measured markup must equal rendered markup" invariant not holding, surfaced during council review. Left unresolved rather than patched around: `fitsOnPage` is one global boolean per recipe, but the prefix is per-cookbook, so there is no single project whose settings would be "correct" to measure with — a recipe can legitimately fit in one cookbook and overflow in another once a prefix is involved. `toggleFavorite` does re-trigger measurement (closing the larger gap, where toggling used to leave a stale `fitsOnPage`), so the common icon-only case stays accurate; only the prefix-pushes-title-to-a-second-line edge case is unmeasured. Documented in `AGENTS.md`/`CLAUDE.md`'s Known Issues.
