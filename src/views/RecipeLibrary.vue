  ...
  <main id="cm-main" class="cm-page-main">
    ...
    <div style="margin-bottom:28px;">
      ...
      <div style="position:relative; max-width:420px;">
        <svg aria-hidden="true" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--gray-52)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="position:absolute; left:13px; top:50%; transform:translateY(-50%);"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg>
        <input id="cm-search" v-model="query" type="text" placeholder="Search by title or ingredient…" class="search-input" />
      </div>
    </div>

    <div v-if="filteredRecipes.length" role="list" aria-label="Recipes" style="background:var(--gray-99); border:1px solid var(--gray-88); border-radius:14px; overflow:hidden;">
      <article
        ...
        class="recipe-row"
        style="display:flex; align-items:flex-start; gap:16px; padding:16px 20px; border-bottom:1px solid var(--gray-88); cursor:pointer;"
    ...
    <div v-else style="border:1px dashed var(--gray-78); border-radius:14px; padding:64px 32px; text-align:center; background:var(--gray-99);">
      ...
      <p style="margin:0 0 20px; font-size:15px; color:var(--gray-46);">{{ recipesStore.recipes.length ? `Nothing matches "${query}". Try a different title or ingredient.` : 'Add your first recipe or import recipes to start building your library.' }}</p>
...
<style scoped>
.btn-import {
  display:flex; align-items:center; gap:8px; background:none; color:var(--gray-20); border:1px solid var(--gray-84); border-radius:8px; padding:12px 18px; font-size:15px; font-weight:600; cursor:pointer; text-decoration:none;
}
.btn-import:hover { background:var(--gray-93); }
.btn-import:focus-visible { outline:2px solid var(--color-focus); outline-offset:2px; }

.btn-new {
  display:flex; align-items:center; gap:8px; background:var(--gray-20); color:var(--gray-99); border:none; border-radius:8px; padding:12px 20px; font-size:15px; font-weight:600; cursor:pointer; text-decoration:none;
}
.btn-new:hover { background:var(--gray-30); }
.btn-new:focus-visible { outline:2px solid var(--color-focus); outline-offset:2px; }

.search-input {
  width:100%; box-sizing:border-box; padding:11px 14px 11px 38px; font-size:15px; border:1px solid var(--gray-84); border-radius:8px; font-family:inherit; background:var(--gray-99);
}
...
.btn-open {
  background:var(--gray-93); border:1px solid var(--gray-84); border-radius:7px; padding:8px 14px; font-size:13px; font-weight:600; cursor:pointer; color:var(--gray-20); white-space:nowrap; text-decoration:none; display:inline-block;
}
.btn-open:hover { background:var(--gray-88); }
.btn-open:focus-visible { outline:2px solid var(--color-focus); outline-offset:2px; }

.btn-icon-danger {
  width:36px; height:36px; flex-shrink:0; display:flex; align-items:center; justify-content:center; background:none; border:1px solid var(--gray-84); border-radius:7px; cursor:pointer; color:oklch(45% 0.05 25);
}
