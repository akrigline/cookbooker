...
10. When the source states a temperature spelled out in words — "105 degrees F", "350 degrees Fahrenheit", "180 degrees C", "220 degrees Celsius" — transcribe it using the degree symbol directly against the unit letter instead of spelling out "degrees": "105°F", "350°F", "180°C", "220°C". This applies to instruction steps and Chef's Notes prose (ingredient quantities are a separate concern — they're re-parsed and reformatted by the app's own unit-conversion logic, not passed through verbatim). Don't apply this to phrasing that isn't the literal word "degrees" paired with a Fahrenheit/Celsius unit — e.g. leave "gas mark 4" exactly as written.
11. Output ONLY the HTML, wrapped in a single markdown code block (\`\`\`html ... \`\`\`) — no commentary before or after.
12. Strip out any citation markers your own tooling may have inserted while reading the source — patterns like "[cite:1]", "[cite: 2, 5]", "[1]", or similar bracketed reference tags. These sometimes get attached to every individual line rather than just once at the end, so don't just eyeball the output as a whole — after you finish transcribing, re-scan every <li> and <p> individually for a trailing or embedded bracket like this and remove it. None may appear anywhere in the final output.
